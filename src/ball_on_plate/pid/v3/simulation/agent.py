...
class BallOnPlate:
    ...
    def perform_action(self) -> bool:
        ...
        if self.mu >= (2/7) * np.tan(self.roll_theta):
            self.ax = (5/7) * self.g * np.sin(self.roll_theta)
        else:
            print(f"Rutschen ROLL")
            self.ax = (5/7) * self.g * (np.sin(self.roll_theta) - self.mu * np.cos(self.roll_theta))

        # Calculate accelerate for y
        if self.mu >= (2/7) * np.tan(self.pitch_theta):
            self.ay = (5/7) * self.g * np.sin(self.pitch_theta)
        else:
            print(f"Rutschen PITCH")
            self.ay = (5/7) * self.g * (np.sin(self.pitch_theta) - self.mu * np.cos(self.pitch_theta))
        ...
        delta_roll = desire_roll - self.roll_theta
        self.roll_theta += np.clip(delta_roll, -max_delta_angle, max_delta_angle)
        ...
        # Set pitch
        desire_pitch = np.clip(uy, -self.max_angle, self.max_angle)
        delta_pitch = desire_pitch - self.pitch_theta
