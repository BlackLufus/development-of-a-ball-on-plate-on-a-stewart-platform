...
class BallOnPlate:
    ...
    def perform_action(self, action) -> bool:
        ...
        self.ax = (5/7) * self.g * np.sin(roll_theta)
        if self.mu >= (2/7) * np.tan(roll_theta):
            self.ax = (5/7) * self.g * np.sin(roll_theta)
        else:
            print(f"Rutschen ROLL")
            self.ax = (5/7) * self.g * (np.sin(roll_theta) - self.mu * np.cos(roll_theta))

        # Calculate accelerate for y
        pitch_theta = math.radians(self.pitch)
        if self.mu >= (2/7) * np.tan(pitch_theta):
            self.ay = (5/7) * self.g * np.sin(pitch_theta)
        else:
            print(f"Rutschen PITCH")
            self.ay = (5/7) * self.g * (np.sin(pitch_theta) - self.mu * np.cos(pitch_theta))
